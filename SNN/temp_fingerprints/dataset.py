...
class SiameseDataset(Dataset):
    def __init__(self, data_dir, transform=None, train=True):
        ...
        # Group images by class/identity
        self.image_classes = {}
        for img_path in self.image_files:
            # Assuming the parent folder name is the class/identity
            class_name = os.path.basename(os.path.dirname(img_path))
            if class_name not in self.image_classes:
                self.image_classes[class_name] = []
            self.image_classes[class_name].append(img_path)
    ...
    def __getitem__(self, idx):
        ...
        anchor_path = self.image_files[idx]
        anchor_class = os.path.basename(os.path.dirname(anchor_path))
        
        # Randomly decide whether to create a positive or negative pair
        should_get_same_class = random.randint(0, 1) if self.train else idx % 2
        
        if should_get_same_class:
            # Get another image from the same class
            positive_path = random.choice(self.image_classes[anchor_class])
            while positive_path == anchor_path:  # Ensure we don't pick the same image
                positive_path = random.choice(self.image_classes[anchor_class])
            second_path = positive_path
            label = 1
        else:
            # Get an image from a different class
            negative_class = random.choice(list(self.image_classes.keys()))
            while negative_class == anchor_class:  # Ensure we pick a different class
                negative_class = random.choice(list(self.image_classes.keys()))
            second_path = random.choice(self.image_classes[negative_class])
            label = 0
            
        # Load and transform images
        img1 = Image.open(anchor_path).convert('L')  # Convert to grayscale
